...
typedef struct {
    ...
    int end_position; // position that the car ended in the race
} car_shm_struct;
...
typedef struct {
    ...
    int nr_cars; // number of cars enrolled in the race
    int end_counter; // incremented when a car finished the race
    bool wait_statistics; // flag to make cars stop for statistics
    car_shm_struct cars[];
} shm_struct;
